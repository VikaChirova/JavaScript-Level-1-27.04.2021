...
var cols = 10, rows = 20; //Размеры поля
var mainCanvas, nextCanvas; 
var fTbl, nTbl; 
var fieldArr = []; //Поле
...
var colors = [ //Массив цветов
 'blue', 'yellow', 'orange', 'grey', 'purple', 'red', 'green',
];
var html5=typeof document.createElement('canvas').getContext === "function";
...
function createField() {//Добавляем на страницу игровое поле и окно для следующей фигуры
 if(html5){
  ...
  get('header').innerHTML = get('header').innerHTML;
 }else{
...
document.body.onkeydown = function(e) {//Это нажатие кнопок на странице
 var keys = { //Клавиши
  37: 'left',
...
function drawRect (x,y,cnv,ctx,c,r) { //рисует прямоугольник
 var w = cnv.width/c;
 var h = cnv.height/r;
...
function rotate(thisShape) { //Вращение текущей фигуры thisShape против часовой стрелки
 ...
 for (var y=0; y<4; y++) {
  newShape[y] = [];
  for (var x=0; x<4; x++) newShape[y][x]=thisShape[3-x][y];
 }
 return newShape;
}
...
function countPlus (lines0) { //Подсчёт очков
 ...
 get('tetriscount').innerHTML = 
  "Линий: "+lines+"<br>Подсчёт: "+count+"<br>Рекорд: "+maxCount;
}
...
function initField() { //Очистить поле
 for (var y=0; y<rows; ++y) {
  fieldArr[y] = [];
...
interval = setInterval (playGame,600); //скорость игры, мс
}
